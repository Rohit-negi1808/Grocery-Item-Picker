...
document.getElementById('algorithm').addEventListener('change', e => {
  document.getElementById('greedyOptions').style.display = e.target.value 
  === 'fractional' ? 'block' : 'none';
});
  ...
  div.innerHTML = `
    ...
    <ul class="result-items">
      ${result.selectedItems.map(item => `
        <li>🛒 ${item.name} 
        (${item.fraction !== 1 ? `${(item.fraction * item.weight).toFixed(2)}kg × ₹${
        (item.fraction * item.value).toFixed(2)}` : `${item.weight}kg, ₹${item.value}`})</li>
      `).join('')}
    </ul>
  `;
}
...
function zeroOneKnapsack(items, maxW) {
  ...
  return { 
    totalValue: dp[n][maxW], totalWeight: maxW - w, selectedItems: selected.reverse()
   };
}
